...
A personal, unpacked Manifest V3 Chrome extension that augments **instagram.com** with four features (see `README.md` for user-facing detail and `PRD.md` for the roadmap):
...
**Load order is significant** and is fixed in `manifest.json` `content_scripts.js`: `config → ig-api → queue → ui → feature1 → feature2 → feature3 → feature4`. A file may only reference `BWI.*` symbols defined by an earlier file. If you add a file or a cross-file dependency, update this order. Shared DOM helpers used by more than one feature (e.g. `ui.dialogTitleIs`, `ui.findScrollContainer`) live in `src/ui.js`.
